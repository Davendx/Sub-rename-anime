...
def calculate_season_episode(absolute_episode, season_data):
    ...
    graph = {season['id']: [] for season in season_data}
    season_map = {season['id']: season for season in season_data}

    for season in season_data:
        for edge in season.get('relations', {}).get('edges', []):
            relation_type = edge.get('relationType')
            node = edge.get('node')
            if node and node['id'] in graph:
                if relation_type == 'SEQUEL':
                    graph[season['id']].append(node['id'])
                elif relation_type == 'PREQUEL':
                    graph[node['id']].append(season['id'])

    # Topological sort to get the chronological order
    ...
    sorted_seasons = [season_map[anime_id] for anime_id in sorted_order]
...
def process_folder(folder_path, files, anime_data, conf, dry_run, force_refresh, interactive, bundle_ova, export_nfo, verbose, rclone_remote=None, rclone_config=None):
    ...
    # Mixed-season detection
    if season_data:
        season_summary = defaultdict(list)
        for file_path in all_files:
            try:
                absolute_episode = int(anitopy.parse(os.path.basename(file_path)).get('episode_number', '0'))
                season, _ = calculate_season_episode(absolute_episode, season_data)
                if season is not None:
                    season_summary[season].append(absolute_episode)
            except (ValueError, TypeError):
                continue

        if len(season_summary) > 1:
            print("\nDetected mixed seasons in this folder:")
            for season, episodes in sorted(season_summary.items()):
                print(f"  - Season {season}: Episodes {min(episodes)}-{max(episodes)}")
    ...
    for file_path in tqdm(all_files, desc=f"Processing files in {os.path.basename(folder_path)}"):
        ...
        if video_file:
            ...
            if export_nfo:
                if rclone_remote:
                    with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".nfo") as temp_nfo:
                        create_nfo_file(temp_nfo.name, anime_data, season, episode, True)
                        remote_nfo_path = os.path.splitext(new_video_path)[0] + ".nfo"
                        rclone_handler.rclone_copyto(temp_nfo.name, remote_nfo_path, rclone_config)
                    os.unlink(temp_nfo.name)
                else:
            ...
            try:
                ...
                if sub_season == season and sub_episode == episode:
                    ...
                    if export_nfo and not video_file:
                        if rclone_remote:
                            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".nfo") as temp_nfo:
                                create_nfo_file(temp_nfo.name, anime_data, season, episode, False)
                                remote_nfo_path = os.path.splitext(new_sub_path)[0] + ".nfo"
                                rclone_handler.rclone_copyto(temp_nfo.name, remote_nfo_path, rclone_config)
                            os.unlink(temp_nfo.name)
                        else:
...
def main():
    ...
    parser.add_argument("--rclone-remotes", help="Path to a text file containing a list of rclone remotes to process.")
    parser.add_argument("--rclone-config", help="Path to the rclone.conf file.")
    parser.add_argument("--mount-path", help="The local path to a mounted rclone remote.")
    ...
    if args.batch:
        try:
            ...
            sys.exit(1)
    elif args.rclone_remotes:
        try:
            with open(args.rclone_remotes, 'r', encoding='utf-8') as f:
                directories = [line.strip() for line in f if line.strip()]
        except IOError as e:
            print(f"Error reading rclone remotes file: {e}")
            sys.exit(1)
    elif args.directory:
        directories.append(args.directory)
    elif args.rclone_remote:
        directories.append(args.rclone_remote)
    elif args.mount_path:
        directories.append(args.mount_path)
    ...
    for directory in directories:
        ...
        for folder, files in file_groups.items():
            ...
            all_files = files['videos'] + files['subtitles']
            parsed_titles = set(
                anitopy.parse(os.path.basename(f)).get('anime_title')
                for f in all_files
                if anitopy.parse(os.path.basename(f)).get('anime_title')
            )

            if not parsed_titles:
                logging.warning(f"Could not parse any titles from the files in '{folder}'. Skipping folder.")
                continue

            search_results = []
            for title in parsed_titles:
                logging.info(f"Searching AniList for '{title}'...")
                search_results.extend(anilist_api.search_anime(title, args.force_refresh))

            # Remove duplicate search results
            unique_results = {anime['id']: anime for anime in search_results}.values()

            selected_anime = None
            if not unique_results:
                print(f"No results found for any of the parsed titles. Proceeding with parsed filenames.")
            elif len(unique_results) == 1:
                selected_anime = list(unique_results)[0]
                ...
                for anime in unique_results:
                    api_titles = [
                        anime['title'].get('romaji'),
                        anime['title'].get('english'),
                        anime['title'].get('native')
                    ] + (anime.get('synonyms') or [])
                    api_titles = [t for t in api_titles if t] # a list of all of the titles from anilist

                    for parsed_title in parsed_titles:
                        match = process.extractOne(parsed_title, api_titles)
                        if match and match[1] > best_match_score:
                            best_match_score = match[1]
                            best_anime = anime

                if best_match_score >= conf['fuzzy_threshold']:
                    selected_anime = best_anime
                    title = selected_anime['title'].get(conf['title_language']) or selected_anime['title'].get('romaji') or selected_anime['title'].get('english') or selected_anime['title'].get('native')
                    print(f"Automatically matched with AniList title (score: {best_match_score}): {title}")
                else:
                    logging.info(f"Fuzzy match score was too low ({best_match_score}). Asking for user input.")
                    selected_anime = choose_anime(list(unique_results))
